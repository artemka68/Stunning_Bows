...
class StunningBowClientEvents {
    //Bow fov while pulling
    @SubscribeEvent
    public static void onComputerFovModifierEvent(ComputeFovModifierEvent event) {
        if(event.getPlayer().isUsingItem() && event.getPlayer().getUseItem().getItem() == SBItems.STUNNING_BOW.get()) {
            float f = 1.0F;
            ...
            float f1 = (float)ticksUsingItem / 14.5F;
            if(f1 > 1.0F) {
                f1 = 1.0F;
            } else {
                f1 *= f1;
